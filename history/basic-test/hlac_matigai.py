...
Original file is located at
    https://colab.research.google.com/drive/1MkhOlTVJVuVp7Y79hProUn8AwLkzNC8A

# 世界一やさしいHLAC入門！(間違い探し編)
Zennにて解説した記事の実装例です。   
https://zenn.dev/kotaro_inoue/articles/f0cbbca962313b

使用するPythonパッケージは以下にまとめました
"""
...
"""使用する間違い探し画像はWikipediaの画像から取得しています。
https://ja.wikipedia.org/wiki/%E9%96%93%E9%81%95%E3%81%84%E6%8E%A2%E3%81%97

全部で15箇所の違いがあるそうです。
"""

# wget https://upload.wikimedia.org/wikipedia/commons/0/0a/Spot_the_difference.png

image = cv2.imread('./my_dir/matigai/Spot_the_difference.png')
# plt.imshow(image)
# plt.show()
...
"""このままでは左右の比較がしづらいので、中央で分割しましょう。"""

r,c = image.shape[:2]
reference = image[:,:int(c/2)]
target = image[:,int(c/2):]
...
diff = cv2.GaussianBlur(diff, (11, 11), 0) # ぼかして小さな誤検出を減らす
# plt.imshow(diff)
# plt.show()
